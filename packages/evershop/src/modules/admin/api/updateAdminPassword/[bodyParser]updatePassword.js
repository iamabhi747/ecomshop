...
const updatePassword = require('../../services/updateAdminPassword');
...
module.exports = async (request, response, delegate, next) => {
  ...
  try {
    ...
    const existingToken = await select()
      .from('reset_password_admin_token')
      .where('token', '=', hash)
      .and('created_at', '>=', now)
      .load(pool);
    ...
    await updatePassword(existingToken.admin_user_id, password, {
      routeId: request.currentRoute.id
    });

    // Delete the token
    await del('reset_password_admin_token')
      .where(
